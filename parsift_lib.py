...
class Reconstruction:
    ...
    def conduct_pca_peripheral(self,full_output=False,image_only=False):
        ...
        self.peripheral_nodes = set(np.fromiter(itertools.chain.from_iterable(self.max_face), dtype=int))
        ...
        if full_output == True or image_only == True:
            ...
            nx.draw_networkx_labels(self.untethered_graph, self.reconstructed_pos, font_family='Times', font_size=6, alpha=0.5)
            plt.autoscale(enable=True, axis='both', tight=None)
            # ipdb.set_trace()
            plt.savefig(self.directory_name + '/' + 'pca_embedding_peripheral.svg')
            plt.savefig(self.directory_name + '/' + 'pca_embedding_peripheral.png')

            vor_reconstructed_spring = Voronoi(convert_dict_to_list(self.reconstructed_pos))
            ...
            plt.savefig(self.directory_name + '/' + 'voronoi_pca_peripheral.svg')
            plt.savefig(self.directory_name + '/' + 'voronoi_pca_peripheral.png')
            plt.close()
    ...
    def conduct_shortest_path_matrix(self,full_output=False,image_only=False):
        ...
        if full_output == True or image_only == True:
            ...
            plt.autoscale(enable=True, axis='both', tight=None)
            # ipdb.set_trace()
            plt.savefig(self.directory_name + '/' + 'pca_embedding_allpairs.svg')
            plt.savefig(self.directory_name + '/' + 'pca_embedding_allpairs.png')
            ...
            for r in range(len(vor_reconstructed_spring.point_region)):
                ...
                if not -1 in region:
                    polygon = [vor_reconstructed_spring.vertices[i] for i in region]
                    plt.fill(*zip(*polygon), color=( self.rgb_stamp_catalog[r][0], self.rgb_stamp_catalog[r][1], self.rgb_stamp_catalog[r][2]))
            plt.savefig(self.directory_name + '/' + 'voronoi_pca_allpairs.svg')
            plt.savefig(self.directory_name + '/' + 'voronoi_pca_allpairs.png')
            plt.close()
    ...
    def conduct_tutte_embedding_from_ideal(self,full_output=False,image_only=False):
        ...
        self.ideal_pos = self.tutte_embedding(self.ideal_graph, self.ideal_max_face)

        if full_output == True:
            plt.close()
            nx.draw_networkx(self.ideal_graph, self.ideal_pos,node_color='r',node_size=25,alpha=0.2,edge_color='r')
            plt.xlim(-1.1,1.1)
            plt.ylim(-1.1, 1.1)
    ...
    def conduct_tutte_embedding(self,full_output=True,image_only=False):
        ...
        for face in range(0, len(faces)):
            if len(faces[face]) > len(self.max_face):
                self.max_face = faces[face]
        self.reconstructed_pos = self.tutte_embedding(self.untethered_graph,
                                              self.max_face)  # need to update self.untethered_graph
        self.reconstructed_points = np.array(convert_dict_to_list(self.reconstructed_pos))
        ###########
        ...
        if full_output == True or image_only == True:
            ...
            maxima = np.max(self.rgb_stamp_catalog)
            self.rgb_stamp_catalog = self.rgb_stamp_catalog / maxima
            vor_reconstructed = Voronoi(convert_dict_to_list(self.updated_reconstructed_pos))
            ...
            for r in range(len(vor_reconstructed.point_region)):
                ...
                if not -1 in region:
                    ...
                    self.rgb_stamp_catalog[r][0], self.rgb_stamp_catalog[r][1], self.rgb_stamp_catalog[r][2]))
            plt.xlim(-1.1, 1.1)
            plt.ylim(-1.1, 1.1)
