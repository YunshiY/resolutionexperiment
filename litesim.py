...
def circular_grid_yunshi(xlen, ylen, randomseed=4):
    #generates a hexagonal lattice with circular boundary
...
def seed(nseed,points,full_output=True):
    npoints = len(points)
...
def rca(p1,points,seed,scaled_imarray, lookup_x_axis, lookup_y_axis,full_output=True):
    npoints = len(points)
    nseed = len(p1)
...
def cheating_alignment(cheating_sheet,bc,group_cell,ideal_graph,rgb_stamp_catalog,full_output=True):
    ...
    if full_output==True: np.savetxt(directory_name+'/'+ 'ajc_polony', ajc_polony, delimiter=",", fmt="%i")
    if group_cell != 0:
        cell_hybrid = list()
    ...
    UntetheredG = nx.Graph()
    for i in range(0, ajc_polony.shape[0]):
        UntetheredG.add_node(i)
        for j in range(0, ajc_polony.shape[1]):
            if ajc_polony[i][j] == 1:
                UntetheredG.add_edge(i, j)
    if group_cell != 0:
        for i in range(0, len(cell_polony)):
            for j in cell_polony[i]:
                UntetheredG.node[int(j)]['cell'] = i
    ...
    if (planarity.is_planar(ideal_graph))==True and nx.is_connected(ideal_graph)==True:
        if full_output == True:
            ...
            ideal_faces = planar_embedding_draw(ideal_graph, title='ideal',full_output=False)
    else:
    ...
    if (planarity.is_planar(UntetheredG))==True and nx.is_connected(UntetheredG)==True:
        if full_output == True:
            faces = planar_embedding_draw(UntetheredG,title='tutte')
        else:
            faces = planar_embedding_draw(UntetheredG, title='tutte',full_output=False)
    else:
        print 'error, graph not fully connected, planarity test failed'
        ipdb.set_trace()
    ...
    for face in range(0,len(faces)):
        if len(faces[face]) > len(max_face):
            max_face = faces[face]
    tutte_pos = tutte_embedding(UntetheredG, max_face)#need to update UntetheredG
    ###########
    #UntetheredG.add_edges_from(triangulation_fix(faces,tutte_pos))
    updated_tutte_pos=tutte_embedding(UntetheredG,max_face)
    if full_output == True:
        nx.draw_networkx(UntetheredG, updated_tutte_pos,node_color='r',node_size=25)
        plt.xlim(-1.1,1.1)
        plt.ylim(-1.1, 1.1)
    ...
    spring_pos = nx.drawing.nx_agraph.pygraphviz_layout(UntetheredG, args='-Goverlap=false')
    if group_cell != 0:
        Border = nx.Graph()
        border_node = list()
        ...
        pos_border = np.zeros((len(UntetheredG.nodes), 2))
        for i in border_node:
            Border.add_node(int(i))
            pos_border[int(i)] = (spring_pos[int(i)])

    UntetheredG.add_edges_from(triangulation_fix(faces, spring_pos))
    if full_output == True:
        plt.figure()
        plt.axis('off')
        ####traingulation######
        if group_cell != 0:
            ...
            nx.draw_networkx_nodes(UntetheredG, spring_pos, node_size=30, cmap=plt.cm.RdYlBu, node_color=polony_cell)

        nx.draw_networkx_edges(UntetheredG, spring_pos, alpha=0.3)
        nx.draw_networkx_labels(UntetheredG, spring_pos, font_family='Times', font_size=6, alpha=0.5)
...
def update_enumerated_faces(enumerated_faces, vsizes_enumerated_faces, partial_face,partial_face_vsize, all_vlines, edgelist,full_output=True):
    face_added = False
...
def planar_embedding_draw(graph, labels=True, title='',full_output=True):
    ...
    for vline in range(0,len(all_vlines)):
        ...
        enumerated_faces,vsizes_enumerated_faces = update_enumerated_faces(enumerated_faces, vsizes_enumerated_faces, partial_face_to_left, partial_face_to_left_vsize, all_vlines, edgelist,full_output=full_output)
        enumerated_faces,vsizes_enumerated_faces = update_enumerated_faces(enumerated_faces, vsizes_enumerated_faces, partial_face_to_right, partial_face_to_right_vsize, all_vlines,edgelist,full_output=full_output)
        face_edge_list = convert_vlines_to_edges(enumerated_faces,edgelist)
...
def align(corseed,reconstructed_points,reconstructed_graph='None',title='alignment',error_threshold = 50,edge_number = 0, full_output = True):
    minerror = 99999999
    if edge_number == 0:
