...
class Surface:
    ...
    def circular_grid_ian(self):
        ...
        for i in range(0, self.box_dim):
            for jj in range(0, self.box_dim):
                self.lattice_sites_a[i * self.box_dim + jj, 0] = float(i) - float(self.roi_radius)
                self.lattice_sites_a[i * self.box_dim + jj, 1] = (float(jj) * math.sqrt(3)) - float(self.roi_radius)
        ...
        for point in self.lattice_sites_both:
    ...
    def crosslink_polonies(self, full_output=True):
        ...
        for i in range(0, self.nsites):
            ...
            self.p2_proxm[i] = np.argsort(self.p2_dis)[0]
            self.p2_secondary[i] = np.argsort(self.p2_dis)[1]
        ...
        if full_output == True:
            ...
            np.savetxt(self.directory_name + '/' + 'cheating_sheet', self.bc_index_pair, delimiter=',')
            plt.rcParams['figure.figsize'] = (20,20)
            if self.nseed < 100:
                for i in range(0, self.nseed):
                    plt.text(self.corseed[i, 0], self.corseed[i, 1], '%s' % i, alpha=0.5)
            plt.xlim(-.1*(np.max(self.site_coordinates[:,0])-np.min(self.site_coordinates[:,0]))+np.min(self.site_coordinates[:,0]),.1*(np.max(self.site_coordinates[:,0])-np.min(self.site_coordinates[:,0]))+np.max(self.site_coordinates[:,0]))
            plt.ylim(-.1*(np.max(self.site_coordinates[:,1])-np.min(self.site_coordinates[:,1]))+np.min(self.site_coordinates[:,1]), .1*(np.max(self.site_coordinates[:,1])-np.min(self.site_coordinates[:,1]))+np.max(self.site_coordinates[:, 1]))
    ...
    def conduct_embeddings(self,full_output=True,image_only=False):
        ideal_graph = get_ideal_graph(self.triangulated_delaunay)
        ...
        if (planarity.is_planar(ideal_graph))==True and nx.is_connected(ideal_graph)==True:
            if full_output == True:
                ideal_faces = self.planar_embedding_draw(ideal_graph,title='ideal')
                plt.savefig(self.directory_name + '/' + 'ideal_planar.png')
            else:
                ideal_faces = self.planar_embedding_draw(ideal_graph, title='ideal',full_output=False)
        else:
        ...
        self.ideal_pos = self.tutte_embedding(ideal_graph, self.ideal_max_face)

        if full_output == True:
            plt.close()
            nx.draw_networkx(ideal_graph, self.ideal_pos,node_color='r',node_size=25,alpha=0.2,edge_color='r')
            plt.xlim(-1.1,1.1)
            plt.ylim(-1.1, 1.1)
            plt.savefig(self.directory_name+'/'+ 'tutte_reconstruction_from_ideal.svg')
            plt.savefig(self.directory_name + '/' + 'tutte_reconstruction_from_ideal.png')
            plt.close()

            vor_reconstructed = Voronoi(convert_dict_to_list(self.ideal_pos))
            voronoi_plot_2d(vor_reconstructed,show_vertices=False,show_points=False)
            plt.savefig(self.directory_name + '/' + 'voronoitutteideal.svg')
            plt.savefig(self.directory_name + '/' + 'voronoitutteideal.png')
            plt.close()
        ...
        if (planarity.is_planar(self.untethered_graph))==True and nx.is_connected(self.untethered_graph)==True:
            if full_output == True:
                faces = self.planar_embedding_draw(self.untethered_graph,title='tutte')
            else:
                faces = self.planar_embedding_draw(self.untethered_graph, title='tutte',full_output=False)
        else:
            print 'error, graph not fully connected, planarity test failed'
            ipdb.set_trace()
        ...
        self.tutte_pos = self.tutte_embedding(self.untethered_graph, self.max_face)#need to update self.untethered_graph
        self.tutte_points = np.array(convert_dict_to_list(self.tutte_pos))
        ###########
        ...
        if full_output == True or image_only == True:
            ...
            plt.savefig(self.directory_name + '/' + 'samseq_tutte_embedding.png')
            plt.close()
            ...
            vor_reconstructed = Voronoi(convert_dict_to_list(self.updated_tutte_pos))
            voronoi_plot_2d(vor_reconstructed,show_vertices=False,show_points=False,line_alpha=0.1)
            for r in range(len(vor_reconstructed.point_region)):
                ...
                if not -1 in region:
                    ...
                    plt.fill(*zip(*polygon), color=( self.rgb_stamp_catalog[r][0], self.rgb_stamp_catalog[r][1], self.rgb_stamp_catalog[r][2]))
            plt.xlim(-1.1,1.1)
            plt.ylim(-1.1, 1.1)
            plt.savefig(self.directory_name + '/' + 'voronoi_image_reconstructed_samseqtutte.svg')
            plt.savefig(self.directory_name + '/' + 'voronoi_image_reconstructed_samseqtutte.png')
            plt.close()

        self.spring_pos = nx.drawing.nx_agraph.pygraphviz_layout(self.untethered_graph, args='-Goverlap=false')
        self.spring_points = np.array(convert_dict_to_list(self.spring_pos))

        self.untethered_graph.add_edges_from(triangulation_fix(faces, self.spring_pos))
        if full_output == True or image_only == True:
            plt.figure()
            plt.axis('off')
            ####traingulation######

            nx.draw_networkx_edges(self.untethered_graph, self.spring_pos, alpha=0.3)
            nx.draw_networkx_labels(self.untethered_graph, self.spring_pos, font_family='Times', font_size=6, alpha=0.5)
            plt.savefig(self.directory_name + '/' + 'samseq_spring_embedding.svg')
            plt.savefig(self.directory_name + '/' + 'samseq_spring_embedding.png')

            vor_reconstructed_spring = Voronoi(convert_dict_to_list(self.spring_pos))
            ...
            for r in range(len(vor_reconstructed_spring.point_region)):
                ...
                if not -1 in region:
                    ...
                    plt.fill(*zip(*polygon), color=( self.rgb_stamp_catalog[r][0], self.rgb_stamp_catalog[r][1], self.rgb_stamp_catalog[r][2]))
            plt.savefig(self.directory_name + '/' + 'voronoi_image_reconstructed_samseqspring.svg')
            plt.savefig(self.directory_name + '/' + 'voronoi_image_reconstructed_samseqspring.png')
            plt.close()

            plt.savefig(self.directory_name+'/'+ 'convexhull.svg')
            plt.close()
        # return spring_pos, updated_tutte_pos, ideal_pos, UntetheredG

    # input: a graph in the form of a dictionary and an outter_face in the form of a list of vertices.
    def tutte_embedding(self,graph, outer_face):
    ...
    def update_enumerated_faces(self,enumerated_faces, vsizes_enumerated_faces, partial_face,partial_face_vsize, all_vlines, edgelist,full_output=True):
        ...
        if len(enumerated_faces) == 0:
            ...
            if full_output==True: 'FACE ADDED via empty start: : : : : ', partial_face, ' to ', enumerated_faces,'candidate edges: ', edgelist[partial_face[0]][0:2], ' and ', edgelist[partial_face[1]][0:2]
        else:
...
class Reconstruction:
    def __init__(self, directory_name, reconstructed_points, reconstructed_graph, corseed):
        self.directory_name = directory_name
        self.reconstructed_points = reconstructed_points
        self.reconstructed_graph = reconstructed_graph
        self.corseed = corseed

    def align(self,title='alignment',error_threshold = 50,edge_number = 0, full_output = True,image_only=False):
        minerror = 99999999
        if edge_number == 0:
        ...
        if full_output == True:
            ...
            if self.reconstructed_graph == 'None':
                ...
                nx.draw_networkx_edges(self.reconstructed_graph,self.reconstructed_points,width=1.0,alpha=0.2,edge_color='r')
                nx.draw_networkx_nodes(self.reconstructed_graph, self.reconstructed_points,node_color='r',node_size=25,alpha=0.2)
